...
def eval():
    ...
    q_network = load_network('red.pt', QNetwork, observation_space, action_space)
    q_network.to(device)

    final_network = load_network('red_final.pt', FinalQNetwork, observation_space, action_space)
    final_network.to(device)

    my_network = load_network('blue.pt', FinalQNetwork, observation_space, action_space)
    ...
    def final_pretrain_policy(env, agent, obs):
        ...
        with torch.no_grad():
            q_values = final_network(observation)
        return torch.argmax(q_values, dim=1).cpu().numpy()[0]
    ...
    print(
        run_eval(
            env=env, red_policy=final_pretrain_policy, blue_policy=my_policy, n_episode=30
        )
    )
    print("=" * 20)
